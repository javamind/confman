...
public class CustomPersistentRememberMeServices extends
    ...
    @Autowired
    private com.ninjamind.confman.repository.PersistentTokenRepository persistentTokenRepository;
    ...
    public CustomPersistentRememberMeServices(Environment env, org.springframework.security.core.userdetails.UserDetailsService userDetailsService) {
        super(env.getProperty("confman.security.rememberme.key"), userDetailsService);
    ...
    private PersistentToken getPersistentToken(String[] cookieTokens) {
        ...
        if (token.getTokenDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate().plusDays(TOKEN_VALIDITY_DAYS).isBefore(LocalDate.now())) {
            persistentTokenRepository.delete(token);
