angular.module('ngAnimateSequence', ['ngAnimate']).factory('$$animateAll', function () {
  ...
  function ($$animateReflow, $animate, $$animateStyler, $$animateAll, $timeout) {
    return function (element, options, queue, duration, completeFn) {
      ...
      var styler = angular.isFunction(options.styler) ? options.styler : angular.isString(options.styler) ? $$animateStyler(options.styler) : $$animateStyler('default');
  ...
  function ($animate, $animateRunner, $sniffer) {
    return function (options) {
      ...
      function addSuffix(target) {
        var styles = 'top left right bottom ' + 'x y width height ' + 'border-width border-radius borderWidth borderRadius' + 'margin margin-top margin-bottom margin-left margin-right ' + 'padding padding-left padding-right padding-top padding-bottom'.split(' ');
...
angular.module('ngAnimateStylers', ['ngAnimateSequence']).config([
  '$$animateStylerProvider',
  function ($$animateStylerProvider) {
    ...
    //Web Animations API
    $$animateStylerProvider.register('webAnimations', [
      '$window',
      '$sniffer',
      function ($window, $sniffer) {
        ...
        return function (element, pre, duration, delay) {
          ...
          var iterations = 1;
          // FIXME(matias): make sure this can be changed
    ...
    $$animateStylerProvider.register('gsap', function () {
      return function (element, pre, duration, delay) {
        ...
        return function (post, done) {
          styler.fromTo(element, (duration || 0) / 1000, pre || {}, angular.extend(post, {
            onComplete: done,
            delay: (delay || 0) / 1000
          }));
        };
...
(function () {
  ...
  function iterator(items, reloop) {
    ...
    function next(it, validate) {
      if (contains(it)) {
        var index = indexOf(it) + 1, found = inRange(index) ? _items[index] : reloop ? first() : null, skip = found && validate && !validate(found);
    ...
    function previous(it, validate) {
      if (contains(it)) {
        var index = indexOf(it) - 1, found = inRange(index) ? _items[index] : reloop ? last() : null, skip = found && validate && !validate(found);
  ...
  var Util = {
      ...
      elementIsSibling: function elementIsSibling(element, otherElement) {
        return element.parent().length && element.parent()[0] === otherElement.parent()[0];
      },
  ...
  function MaterialEffects($animateSequence, $rootElement, $position, $$rAF, $sniffer) {
    var styler = angular.isDefined($rootElement[0].animate) ? 'webAnimations' : angular.isDefined(window['TweenMax'] || window['TweenLite']) ? 'gsap' : angular.isDefined(window['jQuery']) ? 'jQuery' : 'default';
    ...
    function makeSequence(from, to, duration) {
      var animate = $animateSequence({ styler: styler }).animate, sequence = animate(from, to, safeDuration(duration));
      return sequence;
    }
    /**
         *
    ...
    function popIn(element, parentElement, clickElement, done) {
      ...
      if (clickElement) {
        ...
        startPos = translateString(clickPos.left - element[0].offsetWidth / 2, clickPos.top - element[0].offsetHeight / 2, 0) + ' scale(0.2)';
      } else {
        startPos = 'translate3d(0,100%,0) scale(0.5)';
      }
      element.css(self.TRANSFORM, startPos).css('opacity', 0);
      $$rAF(function () {
        $$rAF(function () {
          element.addClass('active').css(self.TRANSFORM, '').css('opacity', '').on(self.TRANSITIONEND_EVENT, finished);
        });
      });
    ...
    function safeVelocity(value) {
      var duration = isNaN(value) ? 0 : Number(value);
      return duration > 100 ? duration / 1000 : duration > 10 ? duration / 100 : duration > 1 ? duration / 10 : duration;
    }
  }
  (function () {
    angular.module('material.animations').directive({
  ...
  angular.module('material.animations').directive('inkRipple', [
    '$materialInkRipple',
    InkRippleDirective
  ...
  function InkRippleService($window, $$rAF, $materialEffects, $timeout) {
    ...
    function attach(element, options) {
      ...
      function createRipple(left, top, positionsAreAbsolute) {
        var rippleEl = angular.element('<div class="material-ripple">').css($materialEffects.ANIMATION_DURATION, options.animationDuration + 'ms').css($materialEffects.ANIMATION_NAME, options.animationName).css($materialEffects.ANIMATION_TIMING, options.animationTimingFunction).on($materialEffects.ANIMATIONEND_EVENT, function () {
            rippleEl.remove();
          });
        if (!rippleContainer) {
        ...
        var css = {
            'background-color': $window.getComputedStyle(rippleEl[0]).color || $window.getComputedStyle(node).color,
  ...
  function MaterialButtonDirective(ngHrefDirectives, $materialInkRipple, $aria) {
    ...
    return {
      ...
      compile: function (element, attr) {
        ...
        innerElement.addClass('material-button-inner').append(element.contents()).on('focus', function () {
          element.addClass('focus');
        }).on('blur', function () {
          element.removeClass('focus');
        });
        element.append(innerElement).attr('tabIndex', -1).on('focus', function () {
          innerElement.focus();
        });
  ...
  /**
     ...
     * Card components.
     */
  angular.module('material.components.card', []).directive('materialCard', [materialCardDirective]);
  /**
     * @ngdoc directive
     * @name materialCard
  ...
  function MaterialCheckboxDirective(inputDirectives, $materialInkRipple, $aria) {
    ...
    return {
      ...
      transclude: true,
      require: '?ngModel',
      template: '<div class="material-container" ink-ripple="checkbox">' + '<div class="material-icon"></div>' + '</div>' + '<div ng-transclude class="material-label"></div>',
      link: postLink
    };
    // **********************************************************
    // Private Methods
  ...
  /**
     ...
     * Scrollable content
     */
  angular.module('material.components.content', ['material.services.registry']).directive('materialContent', [materialContentDirective]);
  /**
     * @ngdoc directive
 ...
 * });
  ...
  function MaterialDialogService($timeout, $materialCompiler, $rootElement, $rootScope, $materialEffects, $animate, $aria) {
    ...
    function showDialog(options) {
      ...
      recentDialog = $materialCompiler.compile(options).then(function (compileData) {
        ...
        var popInTarget = options.targetEvent && options.targetEvent.target && angular.element(options.targetEvent.target);
      ...
      function configureAria(element) {
        ...
        $aria.update(element, { 'role': ROLE.DIALOG });
  ...
  /**
     ...
     * Form
     */
  angular.module('material.components.form', []).directive('materialInputGroup', [materialInputGroupDirective]).directive('materialInput', [materialInputDirective]);
  /**
     * @ngdoc directive
  ...
  /**
     ...
     * Icon
     */
  angular.module('material.components.icon', []).directive('materialIcon', [materialIconDirective]);
  /**
     * @ngdoc directive
     * @name materialIcon
  ...
  /**
     ...
     * List module
     */
  angular.module('material.components.list', []).directive('materialList', [materialListDirective]).directive('materialItem', [materialItemDirective]);
  /**
     * @ngdoc directive
     * @name materialList
  ...
  function materialListDirective() {
    return {
      restrict: 'E',
      link: function ($scope, $element, $attr) {
        $element.attr({ 'role': Constant.ARIA.ROLE.LIST });
      }
    };
  }
  /**
  ...
  function materialItemDirective() {
    return {
      restrict: 'E',
      link: function ($scope, $element, $attr) {
        $element.attr({ 'role': Constant.ARIA.ROLE.LIST_ITEM });
      }
    };
  }
  /**
  ...
  angular.module('material.components.radioButton', [
    'material.animations',
    'material.services.aria'
  ]).directive('materialRadioGroup', [materialRadioGroupDirective]).directive('materialRadioButton', [
    '$aria',
  ...
  function materialRadioGroupDirective() {
    ...
    function link(scope, element, attr, ctrls) {
      var rgCtrl = ctrls[0], ngModelCtrl = ctrls[1] || { $setViewValue: angular.noop };
    ...
    function selectButton(direction, parent, loop) {
      ...
      if (buttons.count()) {
        ...
        var target = !selected ? buttons.first() : direction == 'previous' ? buttons.previous(selected) : buttons.next(selected);
        if (target) {
    ...
    function findAllButtons(element) {
      return Array.prototype.slice.call(element[0].querySelectorAll('material-radio-button'));
    }
    /**
  ...
  function materialRadioButtonDirective($aria) {
    ...
    return {
      ...
      transclude: true,
      template: '<div class="material-container" ink-ripple="checkbox">' + '<div class="material-off"></div>' + '<div class="material-on"></div>' + '</div>' + '<div ng-transclude class="material-label"></div>',
      link: link
    };
  ...
  /**
     ...
     */
  angular.module('material.components.sidenav', ['material.services.registry']).factory('$materialSidenav', [
 ...
 * });
  ...
  /**
     ...
     */
  angular.module('material.components.slider', []).directive('materialSlider', [
    '$window',
  ...
  function MaterialSwitch(checkboxDirectives, radioButtonDirectives) {
    ...
    return {
      restrict: 'E',
      transclude: true,
      template: '<div class="material-switch-bar"></div>' + '<div class="material-switch-thumb">' + radioButtonDirective.template + '</div>',
      require: '?ngModel',
      compile: compile
    };
  ...
  function linkTabInk(scope, element, tabsCtrl, $q, $materialEffects) {
    ...
    var tabsHeader = findNode('.tabs-header-items-container', element);
    // excludes paginators
    ...
    function updateInkBar(immediate) {
      ...
      if (!showInk || isHiding) {
        // no animation
        inkBar.toggleClass('animate', immediate !== true).css({
          display: 'none',
          width: '0px'
        });
      } else {
        ...
        inkBar.toggleClass('animate', immediate !== true).css(styles);
      }
  ...
  function linkTabPagination(scope, element, tabsCtrl, $q, $materialEffects) {
    ...
    function getPageAtTabIndex(tabIndex) {
      ...
      var lastPage = pagination.pagesCount - 1;
      return numPages < 1 ? -1 : tabIndex < 0 ? 0 : tabIndex > lastTab ? lastPage : Math.floor(tabIndex / pagination.itemsPerPage);
    }
    /**
    ...
    function isTabInRange(tabIndex) {
      return tabIndex >= pagination.startIndex && tabIndex <= pagination.endIndex;
    }
  }
  angular.module('material.components.tabs').directive('materialTab', [
    '$attrBind',
    '$aria',
    TabDirective
  ]);
  ...
  function TabDirective($attrBind, $aria) {
    ...
    return {
      ...
      scope: true,
      link: linkTab,
      template: '<material-tab-label ink-ripple ' + 'ng-class="{ disabled : disabled, active : active }"  >' + '</material-tab-label>'
    };
    function linkTab(scope, element, attrs, tabsCtrl, $transclude) {
      ...
      element.on('click', function onRequestSelect() {
        ...
        if (event.which == Constant.KEY_CODE.SPACE)
          tabsCtrl.select(scope);
        else if (event.which === Constant.KEY_CODE.LEFT_ARROW)
          tabsCtrl.previous(scope);
        else if (event.which === Constant.KEY_CODE.RIGHT_ARROW)
          tabsCtrl.next(scope);
      });
  ...
  angular.module('material.components.tabs').factory('$materialTabs', [
    '$materialComponentRegistry',
    TabsService
  ...
  function TabsController($scope, $attrs, $materialComponentRegistry, $timeout) {
    var list = Util.iterator([], false), componentID = 'tabs' + $scope.$id, elements = {}, selected = null, self = this;
    ...
    /**
         ...
         */
    function updateHash() {
      self.$$hash = list.items().map(function (it) {
        return it.$id;
      }).join(',');
    }
    /**
    ...
    function selectTab(tab, noUpdate) {
      if (tab == selected)
        return;
      var activate = makeActivator(true), deactivate = makeActivator(false);
    ...
    function selectTabAt(index, noUpdate) {
      if (list.inRange(index)) {
        var matches = list.findBy('$index', index), it = matches ? matches[0] : null;
        if (it != selected) {
    ...
    function focusOn(which, from) {
      var tab = which === 'current' ? selected : which === 'next' ? list.next(from || selected, isEnabled) : which === 'previous' ? list.previous(from || selected, isEnabled) : null;
  ...
  /**
     ...
     * Tabs
     */
  angular.module('material.components.tabs').directive('materialTabs', [
    '$q',
    '$window',
    '$timeout',
    '$compile',
    ...
    function compileTabsFn() {
      return {
        ...
        post: function tabsPostLink(scope, element, attrs, tabsCtrl, $transclude) {
          ...
          configureAria();
          // Update ARIA values for the Tab group (Tabs)
          alignTabButtons();
          selectDefaultTab();
          // **********************************************************
          // Private Methods
          ...
          function transcludeContentItems() {
            var cntr = findNode('.tabs-content', element), materialViewTmpl = '<div class="material-view" ng-show="active"></div>';
            scope.$watch(getTabsHash, function buildContentItems() {
              var tabs = tabsCtrl.$$tabs(notInCache), views = tabs.map(extractContent);
              ...
              if (views.some(notEmpty)) {
                angular.forEach(views, function (content, j) {
                  var tab = tabs[j++], materialView = $compile(materialViewTmpl)(tab);
            ...
            function enableDisconnect(tab, content) {
              if (!content)
                return;
              var selectedFn = angular.bind(tab, tab.selected), deselectedFn = angular.bind(tab, tab.deselected);
  ...
  var isNodeType = function (node, type) {
    return node.tagName && (node.hasAttribute(type) || node.hasAttribute('data-' + type) || node.tagName.toLowerCase() === type || node.tagName.toLowerCase() === 'data-' + type);
  };
  ...
  var isNodeEmpty = function (node) {
    ...
    return node.nodeType == COMMENT_NODE || node.nodeType == TEXT_NODE && !(node.nodeValue || '').trim();
  };
  ...
  /**
     ...
     * Toast
     */
  angular.module('material.components.toast', ['material.services.compiler']).directive('materialToast', [QpToastDirective]).factory('$materialToast', [
    '$timeout',
    '$rootScope',
    '$materialCompiler',
    '$rootElement',
 ...
 * });
  ...
  function QpToastService($timeout, $rootScope, $materialCompiler, $rootElement, $animate) {
    ...
    function toastOpenClass(position) {
      return 'material-toast-open-' + (position.indexOf('top') > -1 ? 'top' : 'bottom');
    }
    ...
    function showToast(options) {
      ...
      recentToast && recentToast.then(function (destroy) {
        destroy();
      });
    ...
    return {
      ...
      link: function (scope, element, attr) {
        ...
        function setupScrollShrink() {
          ...
          function transform() {
            var translate = y ? 'translate3d(0,' + -y / HEIGHT_FACTOR + 'px, 0)' : '';
            element.css($materialEffects.TRANSFORM, translate);
            contentElement.css('margin-top', y ? -y / HEIGHT_FACTOR + 'px' : '');
          }
        }
      }
    };
  ...
  angular.module('material.decorators', []).config([
    '$provide',
    function ($provide) {
        ...
        $$rAF.debounce = function (cb, invokeApply) {
          ...
          return function debounced() {
            ...
            if (!alreadyQueued) {
              alreadyQueued = true;
              $$rAF(function () {
                invokeApply ? $rootScope.$apply(function () {
  ...
  angular.module('material.services.aria', []).service('$aria', [
    '$log',
    AriaService
  ]);
  ...
  angular.module('material.services.attrBind', []).factory('$attrBind', [
    '$parse',
    '$interpolate',
    MaterialAttrBind
  ]);
  ...
  function MaterialAttrBind($parse, $interpolate) {
    ...
    return function (scope, attrs, bindDefinition, bindDefaults) {
      angular.forEach(bindDefinition || {}, function (definition, scopeName) {
        ...
        var match = definition.match(LOCAL_REGEXP) || [], attrName = match[3] || scopeName, mode = match[1],
          // @, =, or &
          parentGet, unWatchFn;
        switch (mode) {
        case '@':
          // One-way binding from attribute into scope
        ...
        case '=':
          ...
          break;
        case '&':
          // execute an attribute-defined expression in the context of the parent scope
  ...
  angular.module('material.services.compiler', []).service('$materialCompiler', [
    '$q',
    '$http',
    '$injector',
    '$compile',
  ...
  /**
     ...
     */
  angular.module('material.services.position', []).factory('$position', [
    '$document',
    '$window',
  ...
  /**
     ...
     */
  angular.module('material.services.registry', []).factory('$materialComponentRegistry', [
    '$log',
    materialComponentRegistry
  ]);
  /**
  ...
  angular.module('material.services.throttle', []).factory('$throttle', [
    '$timeout',
    '$$q',
    '$log',
  ...
  function MaterialThrottleService($timeout, $$q, $log) {
    ...
    function buildInstance(phases, done, otherwise) {
      var pendingActions = [], cancel = angular.noop, state = STATE_READY;
      ...
      function gotoState(nextState, targetFn) {
        var dfd = $$q.defer(), hasFn = angular.isFunction(targetFn), goNext = hasFn && targetFn.length < 1, fn = hasFn ? targetFn : resolved;
        try {
          state = nextState;
          cancel = fn.apply(null, [goNext ? resolved(dfd) : hasFn ? callbackToResolve(dfd) : dfd]);
        } catch (error) {
          dfd.reject(error);
        }
        return dfd.promise;
